...
public class CategoryManagementPanel extends CrudPanelTemplate {
    ...
    private final List<Category> displayedData = new ArrayList<>();
    ...
    public CategoryManagementPanel() {
        ...
        initLayout("Quản lý thể loại", "Tạo và tổ chức danh mục sách", new String[]{"ID", "Tên thể loại", "Mô tả"}, buildForm());
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                makeTransparentAndApplyGlass(CategoryManagementPanel.this);
                // Ép giao diện vẽ lại một lần nữa để xóa sạch nền trắng
                CategoryManagementPanel.this.revalidate();
                CategoryManagementPanel.this.repaint();
            }
        });
    }
    ...
    private void makeTransparentAndApplyGlass(Container parent) {
        for (Component c : parent.getComponents()) {
            ...
            // 1. Loại trừ các thành phần nhập liệu & nút bấm
            ...
            // 4. Ép luôn cái Bảng (JTable) tàng hình
            ...
            if (c instanceof JPanel) {
                ...
                // Chỉnh chiều cao quét xuống 100 vì Form Thể Loại khá nhỏ gọn
    ...
    private JPanel buildForm() {
        ...
        addRow(form, gbc, 0, "Tên thể loại", txtName);
        txtDescription.setLineWrap(true);
        txtDescription.setWrapStyleWord(true);
        addRow(form, gbc, 1, "Mô tả", UIHelper.wrapArea(txtDescription));
    ...
    protected void deleteEntity() {
        if (current == null || current.getId() == null) {
            throw new IllegalArgumentException("Vui lòng chọn thể loại cần xóa");
        }
        if (UIHelper.confirm(this, "Bạn có chắc chắn muốn xóa thể loại này?")) {
